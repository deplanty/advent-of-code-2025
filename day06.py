...
with Reader(6, example=False) as reader:
    numbers: list[list[int]] = list() # type: ignore
    operators: list[str] = list() # type: ignore
    line: str
    for line in reader.iter_lines():
        line = line.lstrip(" ")
        elements: list[str] = re.split(r"\s+", line)
        if elements[0].isnumeric():
            numbers.append([int(x) for x in elements]) # type: ignore
        else:
            operators = elements # type: ignore
...
for index in range(len(operators)):
    ...
    for i in range(len(numbers)):
        column = eval(f"column {op} {numbers[i][index]}") # type: ignore
    total += column
